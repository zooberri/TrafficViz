...
#python detect.py --weights best.pt --conf 0.25 --img-size 640 --source inference/videos/carsvidshort.mp4
...
def detect(save_img=False):
    ...
    for path, img, im0s, vid_cap in dataset:
        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            #count total bounding boxes in image and print
            for i, det in enumerate(pred):  # detections per image
                if len(det):
                    b_box += len(det)
            #printing b_box for testing 
            #print('BBOX COUNTED : ' , b_box)
            ...
            #reset bounding box amount for next iteration
            b_box = 0
            time.sleep(.5)
            # Stream results
            if view_img:
